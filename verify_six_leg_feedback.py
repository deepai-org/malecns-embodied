...
def main():
    ...
    a=p.parse_args();root=Path(__file__).parent;trial=root/'evidence/trials/six-leg-feedback-001'
    r=json.loads((trial/'result.json').read_text());body=json.loads((root/'evidence/six-leg-body-001.json').read_text())
    assert r['completed'] and r['runner_sha256']==sha(root/'run_six_leg_feedback.py')
    assert r['input_sha256']['body_receipt']==sha(root/'evidence/six-leg-body-001.json')
    ...
    result=dict(checked=True,verifier_sha256=sha(Path(__file__)),maximum_control_reconstruction_error=maximum,
        ...
        caveat='Checks recorded transforms, not behavioral usefulness, calibrated anatomy or complete state replay; muscle activation states were not recorded.')
