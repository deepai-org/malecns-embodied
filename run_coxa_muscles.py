"""Physics-step antagonist forces coupled to the full CNS; no gait controller.
...
def main():
    ...
    p.add_argument('--coxa-map',type=Path,required=True)
    p.add_argument('--sensory',type=Path,required=True)
    p.add_argument('--sensory-polarity',type=int,choices=(-1,1),required=True)
    ...
    coxa_map = json.loads(a.coxa_map.read_text())
    drive = CoxaLegMuscles(annotation,arrays['atlas.motor_rows'],scene,coxa_map,a.polarity)
    sensory = json.loads(a.sensory.read_text())
    if sensory['source_sha256'] != annotation['source_sha256']:
        raise ValueError('sensory/motor annotation mismatch')
    sensor = NamedLegPosition(sensory,scene,arrays['atlas.body_rows'],arrays['atlas.motor_rows'],a.sensory_polarity)
    binary = a.source/'native/fly-world/target/release/chreatures-fly-world'
    identity = dict(coxa_map_sha256=hashlib.sha256(a.coxa_map.read_bytes()).hexdigest(),
                    base_muscle_code_sha256=hashlib.sha256(Path(__file__).with_name('aggregate_leg_muscles.py').read_bytes()).hexdigest(),
                    sensory_groups=sensor.groups, missing_sensory_groups=sensor.missing,
                    sensory_polarity=a.sensory_polarity, sensory_amplitude=.2,
                    sensory_annotation_sha256=hashlib.sha256(a.sensory.read_bytes()).hexdigest(),
                    sensory_adapter_sha256=hashlib.sha256(Path(__file__).with_name('named_leg_senses.py').read_bytes()).hexdigest(),
                    condition=a.condition,polarity=a.polarity,steps=a.steps,
                    force_update_seconds=.0001,
                    force_adapter_sha256=hashlib.sha256(Path(__file__).with_name('physics_muscles.py').read_bytes()).hexdigest(),
                    model_manifest_sha256=model_hash,
                    runner_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
                    muscle_code_sha256=hashlib.sha256(Path(__file__).with_name('coxa_leg_muscles.py').read_bytes()).hexdigest(),
                    ...
                    caveats=['36 aggregate muscle units across 18 hinges; 66 other joint actuators unpowered.',
                             ...
                             'Named claw rows replace broad sensory projection; other sensory rows retain it. No motor decoder.',
                             'Claw transduction uses rig-centered tanh position, assumed 1-radian scale and 0.2 amplitude.',
                             'Coordinate polarity is unvalidated; missing groups are not synthesized.',
                             'Coxa actuation is a fixed one-axis approximation, not measured muscle geometry.',
                             'Coefficients fixed per neural tick; physical force refreshed every physics step.',
                             'Ecology receives zero high-level command; its energetic bookkeeping remains unvalidated.',
    ...
    history = {k:[] for k in ('named_sensory_offsets','time','qpos','qvel','optic','body','motor_rates','nominal_held_command','muscle_coefficients','activation','actuator_force','thorax_position','thorax_up')}
    world = None
    ...
    try:
        with (a.output/'world.stderr.log').open('w') as log:
            ...
            ready = world.receive()
            if ready.get('experimental_muscle_mode') != 'affine-force-physics-step-v1':
                raise ValueError('wrong physics-step muscle host')
            count = ready['residents']
            if count != len(scene['residents']):
                raise ValueError('resident count changed')
            state = cns.initial_state(count)
            neutral = cns.neutral_current(count).contiguous()
            ...
            with torch.inference_mode():
                for tick in range(a.steps+1):
                    ...
                    if tick == 0:
                        ...
                        frozen_qpos = qpos.copy()
                    if tick == a.steps:
                        break
                    inputs = (frozen_optic,frozen_body) if a.condition=='frozen-sensory' else (optic,body)
                    current = cns.afferent_current(torch.as_tensor(inputs[0],device='cuda:0'),
                                                   torch.as_tensor(inputs[1],device='cuda:0'),context)
                    offsets = sensor.offsets(frozen_qpos if a.condition=='frozen-sensory' else qpos)
                    for group, values in zip(sensor.groups, offsets):
                        current[group['rows']] = neutral[group['rows']] + torch.as_tensor(values,device='cuda:0')[None]
                    history['named_sensory_offsets'].append(offsets)
                    ...
                    history['nominal_held_command'].append(command.copy())
                    coefficients = muscle_coefficients(drive)
                    history['muscle_coefficients'].append(coefficients.copy())
                    history['activation'].append(drive.activation.copy())
                    world.rpc('advance_muscle',muscle252_base64=base64.b64encode(coefficients.tobytes()).decode())
                    if tick%50 == 0:
        ...
        result = dict(identity,completed=True,wall_seconds=time.monotonic()-began,
                      ...
                      maximum_command=float(np.max(np.abs(history['nominal_held_command']))),behavior_claim='none')
