...
def main():
    ...
    mapping=json.loads((root/'evidence/coxa-map-001.json').read_text())
    assert mapping['scene_sha256']==sha(a.source_scene)
    assert mapping['runner_sha256']==sha(root/'map_coxa_protraction.py')
    traces={}; reports={}
    for mode in ('driven','frozen','zero'):
        directory=root/f'evidence/trials/coxa-{mode}-001'
        ...
        for key,file in [('runner_sha256','run_coxa_muscles.py'),('coxa_map_sha256','evidence/coxa-map-001.json'),('base_muscle_code_sha256','aggregate_leg_muscles.py'),('sensory_adapter_sha256','named_leg_senses.py'),
                         ('sensory_annotation_sha256','proprioceptor-annotations.json'),('muscle_code_sha256','coxa_leg_muscles.py'),
        ...
        drive=CoxaLegMuscles(motor,motor_rows,scene,mapping,r['polarity'])
        assert len(drive.groups)==18
        ...
        reports[mode]=dict(simulated_seconds=float(t['time'][-1]),
            actuated_joints=len(drive.groups),
    ...
    report=dict(checked=True,verifier_sha256=sha(Path(__file__)),results=reports,
        driven_vs_frozen_max_root_difference_mm=float(np.linalg.norm(traces['driven']['thorax_position']-traces['frozen']['thorax_position'],axis=2).max()),
